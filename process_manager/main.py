"""
Seminario de Solución de Problemas de Sistemas Operativos - D05 - 12/09/2023
Actividad #4 - Administrador de Procesos
...
class process_manager(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        self.processes = (
            (self.token_0, self.user_0, self.state_0, self.progress_0),
            (self.token_1, self.user_1, self.state_1, self.progress_1),
            (self.token_2, self.user_2, self.state_2, self.progress_2),
            (self.token_3, self.user_3, self.state_3, self.progress_3),
            (self.token_4, self.user_4, self.state_4, self.progress_4),
            (self.token_5, self.user_5, self.state_5, self.progress_5),
            (self.token_6, self.user_6, self.state_6, self.progress_6),
            (self.token_7, self.user_7, self.state_7, self.progress_7)
        )

        # Inicializamos los procesos.

        for process in self.processes:
            process[0].setText(str(random.randint(30, 120)))
            process[2].setText('Initializing...')
    ...
    def get_command(self) -> None:
        tokens = self.prompt.text().split() # Separamos el comando.
